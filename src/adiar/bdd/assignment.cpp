  ...
  public:
    ...
    bdd::ptr_t visit(const bdd::node_t &n)
    {
      ...
      while (lvls.peek().label() < label) {
        aw << assignment(lvls.pull().label(), skipped_value);
      }
      adiar_debug(lvls.peek().label() == label,
                  "level given should exist in BDD");

      aw << assignment(lvls.pull().label(), next_ptr == n.high());
      return next_ptr;
    }
    ...
    const shared_file<assignment> get_result() const
    {
      return af;
    }
  };
