...
go_bandit([]() {
  describe("adiar/internal/dd_func.h", []() {
    ...
    describe("bdd_topvar / zdd_topvar", [&]() {
      ...
      it("returns shifted root variable for larger ZDD [+1]", [&]() {
        AssertThat(zdd_topvar(zdd(zdd_file, false, +1)), Is().EqualTo(1u));
        AssertThat(zdd_minvar(zdd(zdd_file, false, +1)), Is().EqualTo(1u));
      });

      it("returns shifted root variable for larger ZDD [+2]", [&]() {
        AssertThat(zdd_topvar(zdd(zdd_file, false, +2)), Is().EqualTo(2u));
        AssertThat(zdd_minvar(zdd(zdd_file, false, +2)), Is().EqualTo(2u));
      });
    });
    ...
    describe("bdd_maxvar / zdd_maxvar", [&]() {
      ...
      it("returns shifted root variable for larger ZDD [+1]", [&]() {
        AssertThat(zdd_maxvar(zdd(zdd_file, false, +1)), Is().EqualTo(3u));
      });

      it("returns shifted root variable for larger ZDD [+2]", [&]() {
        AssertThat(zdd_maxvar(zdd(zdd_file, false, +2)), Is().EqualTo(4u));
      });
    });
  });
});
