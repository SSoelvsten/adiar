  ...
  describe("adiar/internal/data_types/request.h", []() {
    ...
    describe("request<cardinality = 1>", []() {
      ...
      describe(".targets()", []() {
        ...
        it("is 1 for (MAX,MAX) target", []() {
          const request<1> req(request<1>::pointer_type(request<1>::pointer_type::max_label,request<1>::pointer_type::max_id), {});
          AssertThat(req.targets(), Is().EqualTo(1));
        });
    ...
    describe("request<cardinality = 2>", []() {
      ...
      describe(".level()", []() {
        it("returns the level of the first uid in target [1]", []() {
          const request<2> req({request<2>::pointer_type(0u,1u), request<2>::pointer_type(1u,0u)}, {});
          AssertThat(req.level(), Is().EqualTo(0u));
        });

        it("returns the level of the first uid in target [2]", []() {
          const request<2> req({request<2>::pointer_type(1u,0u), request<2>::pointer_type(0u,1u)}, {});
          AssertThat(req.level(), Is().EqualTo(0u));
        });

        it("returns the level of the first uid in target [3]", []() {
          const request<2> req({request<2>::pointer_type(21u,11u), request<2>::pointer_type(42u,8u)}, {});
          AssertThat(req.level(), Is().EqualTo(21u));
        });

        it("returns the level of the first uid in target [4]", []() {
          const request<2> req({request<2>::pointer_type(42u,11u), request<2>::pointer_type(21u,42u)}, {});
          AssertThat(req.level(), Is().EqualTo(21u));
        });
      });
      ...
      describe(".nodes_carried()", []() {
        it("has no nodes when node_carry_size is 0", []() {
          const request<2,0> req({request<2>::pointer_type(1u,1u), request<2>::pointer_type(1u,0u)}, {});
          AssertThat(req.nodes_carried(), Is().EqualTo(0u));
        });

        it("has no nodes when node_carry_size is 1 with manually added nil()", []() {
          const request<2,1> req({request<2>::pointer_type(1u,1u), request<2>::pointer_type(1u,0u)},
                                 {{ request<2>::pointer_type::nil() }});
          AssertThat(req.nodes_carried(), Is().EqualTo(0u));
        });
      ...
      describe(".targets()", []() {
        ...
        it("is 0 for {nil, nil} target [sorted]", []() {
          const request<2,0,1> req({request<2>::pointer_type::nil(), request<2>::pointer_type::nil()}, {});
          AssertThat(req.targets(), Is().EqualTo(0));
        });
        ...
        it("is 1 for {(0,0), nil} target [sorted]", []() {
          const request<2,0,1> req({request<2>::pointer_type(0,0), request<2>::pointer_type::nil()}, {});
          AssertThat(req.targets(), Is().EqualTo(1));
        });
        ...
        it("is 1 for {T, nil} target [sorted]", []() {
          const request<2,0,1> req({request<2>::pointer_type(true), request<2>::pointer_type::nil()}, {});
          AssertThat(req.targets(), Is().EqualTo(1));
        });
        ...
        it("is 2 for {(0,0), T} target [sorted]", []() {
          const request<2,0,1> req({request<2>::pointer_type(0,0), request<2>::pointer_type(true)}, {});
          AssertThat(req.targets(), Is().EqualTo(2));
        });
      });
    });
    ...
    describe("request<cardinality = 3>", []() {
      ...
      it("has default node carry size of 0 (16 bytes)", []() {
        ...
        const request<3,0> rec({ request<3>::pointer_type(0,0), request<3>::pointer_type(0,0), request<3>::pointer_type(0,0) },
                               { });
      ...
      it("can have a node carry size of 1 (32 bytes)", []() {
        ...
        const request<3,1> rec({ request<3>::pointer_type(0,0), request<3>::pointer_type(0,0), request<3>::pointer_type(0,0) },
      ...
      it("can have a node carry size of 2 (48 bytes)", []() {
        ...
        const request<3,2> rec({ request<3>::pointer_type(0,0), request<3>::pointer_type(0,0), request<3>::pointer_type(0,0) },
                               {{ {request<3>::pointer_type::nil()},
                                  {request<3>::pointer_type::nil()} }});
      ...
      describe(".level()", []() {
        it("returns the level of the first uid in target [1]", []() {
          const request<3> req({request<3>::pointer_type(0u,2u), request<3>::pointer_type(1u,1u), request<3>::pointer_type(2u,0u)}, {});
          AssertThat(req.level(), Is().EqualTo(0u));
        });

        it("returns the level of the first uid in target [2]", []() {
          const request<3> req({request<3>::pointer_type(1u,1u), request<3>::pointer_type(0u,2u), request<3>::pointer_type(2u,0u)}, {});
          AssertThat(req.level(), Is().EqualTo(0u));
        });


        it("returns the level of the first uid in target [3]", []() {
          const request<3> req({request<3>::pointer_type(1u,1u), request<3>::pointer_type(2u,0u), request<3>::pointer_type(0u,2u)}, {});
          AssertThat(req.level(), Is().EqualTo(0u));
        });

        it("returns the level of the first uid in target [4]", []() {
          const request<3> req({request<3>::pointer_type(32u,0u), request<3>::pointer_type(21u,11u), request<3>::pointer_type(42u,8u)}, {});
          AssertThat(req.level(), Is().EqualTo(21u));
        });

        it("returns the level of the first uid in target [4]", []() {
          const request<3> req({request<3>::pointer_type(32u,0u), request<3>::pointer_type(42u,11u), request<3>::pointer_type(21u,42u)}, {});
          AssertThat(req.level(), Is().EqualTo(21u));
        });
      });
      ...
      describe(".nodes_carried()", []() {
        it("has no nodes when node_carry_size is 0", []() {
          const request<3,0> req({request<3>::pointer_type(1u,1u), request<3>::pointer_type(1u,0u), request<3>::pointer_type(1u,2u)},
                                 {});
          AssertThat(req.nodes_carried(), Is().EqualTo(0u));
        });

        it("has no nodes when node_carry_size is 1 with manually added nil()", []() {
          const request<3,1> req({request<3>::pointer_type(1u,1u), request<3>::pointer_type(1u,0u), request<3>::pointer_type(1u,2u)},
                                 {{ {request<3>::pointer_type::nil()} }});
          AssertThat(req.nodes_carried(), Is().EqualTo(0u));
        });

        it("has no nodes when node_carry_size is 2 with manually added nil()", []() {
          const request<3,2> req({request<3>::pointer_type(1u,1u), request<3>::pointer_type(1u,0u), request<3>::pointer_type(1u,2u)},
                                 {{ {request<3>::pointer_type::nil()},
                                    {request<3>::pointer_type::nil()} }});
          AssertThat(req.nodes_carried(), Is().EqualTo(0u));
        });
      ...
      describe(".targets()", []() {
        it("is 0 for {nil, nil, nil} target [unsorted]", []() {
          const request<3>req({request<3>::pointer_type::nil(), request<3>::pointer_type::nil(), request<3>::pointer_type::nil()}, {});
          AssertThat(req.targets(), Is().EqualTo(0));
        });

        it("is 0 for {nil, nil, nil} target [sorted]", []() {
          const request<3,0,1> req({request<3>::pointer_type::nil(), request<3>::pointer_type::nil(), request<3>::pointer_type::nil()}, {});
          AssertThat(req.targets(), Is().EqualTo(0));
        });

        it("is 1 for {nil, T, nil} target [unsorted]", []() {
          const request<3> req({request<3>::pointer_type::nil(), request<3>::pointer_type(true), request<3>::pointer_type::nil()}, {});
          AssertThat(req.targets(), Is().EqualTo(1));
        });

        it("is 1 for {(1,0), nil, nil} target [unsorted]", []() {
          const request<3> req({request<3>::pointer_type(1,0), request<3>::pointer_type::nil(), request<3>::pointer_type::nil()}, {});
          AssertThat(req.targets(), Is().EqualTo(1));
        });

        it("is 1 for {nil, nil, (0,0)} target [unsorted]", []() {
          const request<3> req({request<3>::pointer_type::nil(), request<3>::pointer_type::nil(), request<3>::pointer_type(0,0)}, {});
          AssertThat(req.targets(), Is().EqualTo(1));
        });

        it("is 1 for {T, nil, nil} target [sorted]", []() {
          const request<3,0,1> req({request<3>::pointer_type(true), request<3>::pointer_type::nil(), request<3>::pointer_type::nil()}, {});
          AssertThat(req.targets(), Is().EqualTo(1));
        });

        it("is 2 for {T, nil, (0,0)} target [unsorted]", []() {
          const request<3> req({request<3>::pointer_type(true), request<3>::pointer_type::nil(), request<3>::pointer_type(0,0)}, {});
          AssertThat(req.targets(), Is().EqualTo(2));
        });

        it("is 2 for {nil, (42,0), (0,0)} target [unsorted]", []() {
          const request<3> req({request<3>::pointer_type::nil(), request<3>::pointer_type(42,0), request<3>::pointer_type(0,0)}, {});
          AssertThat(req.targets(), Is().EqualTo(2));
        });

        it("is 2 for {(42,0), (0,0), nil} target [unsorted]", []() {
          const request<3> req({request<3>::pointer_type(42,0), request<3>::pointer_type(0,0), request<3>::pointer_type::nil()}, {});
          AssertThat(req.targets(), Is().EqualTo(2));
        });

        it("is 2 for {(2,8), F, nil} target [sorted]", []() {
          const request<3,0,1> req({request<3>::pointer_type(2,8), request<3>::pointer_type(false), request<3>::pointer_type::nil()}, {});
          AssertThat(req.targets(), Is().EqualTo(2));
        });

        it("is 3 for {(2,8), F, (3,0)} target [unsorted]", []() {
          const request<3> req({request<3>::pointer_type(2,8), request<3>::pointer_type(false), request<3>::pointer_type(3,0)}, {});
          AssertThat(req.targets(), Is().EqualTo(3));
        });

        it("is 3 for {(2,8), (3,0), T} target [sorted]", []() {
          const request<3,0,1> req({request<3>::pointer_type(2,8), request<3>::pointer_type(3,0), request<3>::pointer_type(true)}, {});
          AssertThat(req.targets(), Is().EqualTo(3));
        });
      });
    });
