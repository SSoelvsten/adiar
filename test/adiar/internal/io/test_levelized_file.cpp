...
go_bandit([]() {
  describe("adiar/internal/io/levelized_file.h , levelized_file_stream.h , levelized_file_writer.h", []() {
    ...
    describe("levelized_file()", [&tmp_path, &curr_path]() {
      ...
      it("cannot be 'moved' on-top of the path prefix itself [/tmp/]", [&tmp_path]() {
        ...
        if (std::filesystem::exists(path_prefix))
          std::filesystem::remove(path_prefix);
      });
    ...
    describe("levelized_file(path_prefix)", [&tmp_path, &curr_path]() {
      ...
      it("throws exception on path-prefix to partially non-existing files [missing .file_0]", [&curr_path]() {
        const std::string path_prefix = curr_path + "non-existing-prefix.adiar";
      ...
      it("throws exception on path-prefix to partially non-existing files [missing .file_1]", [&curr_path]() {
        const std::string path_prefix = curr_path + "non-existing-prefix.adiar";
        { // Create the 'existing' files by hand (and clean up after prior tests)
          // TODO: take the respective file headers into account
          if (std::filesystem::exists(path_prefix+".file_0"))
      ...
      it("throws exception on path-prefix to partially non-existing files [missing .levels]", [&curr_path]() {
        const std::string path_prefix = curr_path + "non-existing-prefix.adiar";
        { // Create the 'existing' files by hand (and clean up after prior tests)
          // TODO: take the respective file headers into account
          if (std::filesystem::exists(path_prefix+".file_0"))
      ...
      it("can reopen a persisted non-empty file", [&path_prefix]() {
        levelized_file<int> lf(path_prefix);
      });
