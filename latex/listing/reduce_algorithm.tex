\begin{blstlisting}
    Reduce($\ReduceLforward$: Edge[], $\ReduceQdep$: PriorityQueue<Edge>): Node[]
      ...
      for j := n-1 downto 2:
          ...
          // Notice, to-be outputted nodes in $\color{cGray}L_{j,\mathit{out}}$ and rule-2-reduced nodes
          // nodes in $\color{cGray}L_{j,\mathit{red:}2}$ are already sorted in decreasingly order
          // according to i.
          ...
          { source, is_high, target } := $\ReduceLdep$[iF]
          for (w,w',_) in $L_{j:F}$:
              while target = w:
                  $\ReduceQdep$.insert({
                    source=source,
